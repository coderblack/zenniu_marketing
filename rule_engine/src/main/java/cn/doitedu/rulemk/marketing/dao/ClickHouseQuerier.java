...
public class ClickHouseQuerier {
     ...
     */
    // TODO 为了确保bug被修复，此方法改为private，确保没有任何上层模块可以调用本底层方法
    private String getEventCombinationConditionStr(String keyByValue,
        ...
        preparedStatement.setString(1,keyByValue);
        log.debug("clickhouse查询前 ,preparestatement 设置参数1： {}" ,keyByValue);
    ...
    /**
     *  TODO 为了修复bug，下面的方法加了一个参数
     *  此处为了保留原来的上层代码不用更改，因此保留此方法
     * @param keyByValue
     ...
     * @throws SQLException
     */
    public Tuple2<String,Integer> queryEventCombinationConditionCount(String keyByValue,
                                                                      EventCombinationCondition eventCombinationCondition,
                                                                      long queryRangeStart,
                                                                      long queryRangeEnd) throws SQLException {
        return queryEventCombinationConditionCount(keyByValue,eventCombinationCondition,queryRangeStart,queryRangeEnd,false);
    ...
    public Tuple2<String,Integer> queryEventCombinationConditionCount(String keyByValue,
                                                                      ...
                                                                      long queryRangeStart,
                                                                      long queryRangeEnd,
                                                                      boolean needWholeStr) throws SQLException {
        ...
        for (String key : keySet) {
            ...
            if(queryRangeEnd == bufferEndTime && queryRangeStart < bufferStartTime){
                ...
                if(bufferCount>=queryMinCount && !needWholeStr) {
                    ...
                    // 调整查询时间，去clickhouse中查一小段
                    String leftStr = getEventCombinationConditionStr(keyByValue, eventCombinationCondition, queryRangeStart, bufferStartTime);
